...
void fluxCalc_Ele(Model_Data DS, int i,realtype t)
	{
		realtype S,Rn,G,T,Vel,RH,VP,LAI,rl,P,Gamma,Delta,r_a,Rmax,alpha_r,f_r,eta_s,elemSatn,beta_s,r_s,TotalY_unsat,effK_unsat,satKfunc,tmpVar,tmpVar2,effKI,AquiferDepth,pet,mf;
...
*/
                        ...
                        if(AquiferDepth-DS->DummyY[i_plus_2ele]<DS->Ele[i].RzD)
                                {
                                elemSatn=1.0;
				beta_s=elemSatn;
...
//                        	DS->EleET[i][1] = DS->pcCal.Et1*((1-pow(((DS->EleIS[i]+DS->EleSnowCanopy[i]<0)?0:(DS->EleIS[i]+DS->EleSnowCanopy[i]))/(DS->EleISmax[i]+DS->EleISsnowmax[i]),2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s/r_a))));
				tmpVar=(DS->EleIS[i]+DS->EleSnowCanopy[i])/(DS->EleISmax[i]+(DS->EleSnowCanopy[i]>0?1.0:0)*DS->EleISsnowmax[i]);
                        	DS->EleET[i][1] = DS->pcCal.Et1*(((DS->EleIS[i]+DS->EleSnowCanopy[i])>0)?(tmpVar>1.0?0:1-pow(tmpVar,2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s/r_a))):0);
				...
				beta_s= ((elemSatn<EPS_4)?EPS_4:((elemSatn>1.0)?1:elemSatn));
                       		r_s=((DS->Ele[i].Rmin*alpha_r/(beta_s*LAI*pow(eta_s,4)))> Rmax)?Rmax:(DS->Ele[i].Rmin*alpha_r/(beta_s*LAI*pow(eta_s,4)));
				tmpVar=(DS->EleIS[i]+DS->EleSnowCanopy[i])/(DS->EleISmax[i]+(DS->EleSnowCanopy[i]>0?1.0:0)*DS->EleISsnowmax[i]);
                        	DS->EleET[i][1] = ((beta_s<=EPS_4)?0:1.0)*DS->pcCal.Et1*(((DS->EleIS[i]+DS->EleSnowCanopy[i])>0)?(tmpVar>1.0?0:1-pow(tmpVar,2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s/r_a))):0);
//                        	DS->EleET[i][1] = ((beta_s<=EPS_4)?0:1.0)*DS->pcCal.Et1*((1-pow(((DS->EleIS[i]+DS->EleSnowCanopy[i]<0)?0:(DS->EleIS[i]+DS->EleSnowCanopy[i]))/(DS->EleISmax[i]+DS->EleISsnowmax[i]),2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s/r_a))));
				...
				elemSatn1 =DS->DummyY[i_plus_ele]/DS->Ele[i].infD;
                                elemSatn2 =DS->DummyY[i_plus_3ele]/(((AquiferDepth-DS->Ele[i].infD-DS->DummyY[i_plus_2ele])<DS->Ele[i].infD)?DS->Ele[i].infD:(AquiferDepth-DS->Ele[i].infD-DS->DummyY[i_plus_2ele]));
                                ...
                                r_s=(r_s1+r_s2)/2;
				tmpVar=(DS->EleIS[i]+DS->EleSnowCanopy[i])/(DS->EleISmax[i]+(DS->EleSnowCanopy[i]>0?1.0:0)*DS->EleISsnowmax[i]);
                        	DS->EleET[i][1] = (beta_s1<=EPS_4)?((beta_s2<=EPS_4)?0:(DS->pcCal.Et1*(((DS->EleIS[i]+DS->EleSnowCanopy[i])>0)?(tmpVar>1.0?0:1-pow(tmpVar,2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s2/r_a))):0))):((beta_s2<=EPS_4)?(DS->pcCal.Et1*(((DS->EleIS[i]+DS->EleSnowCanopy[i])>0)?(tmpVar>1.0?0:1-pow(tmpVar,2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s1/r_a))):0)):(DS->pcCal.Et1*(((DS->EleIS[i]+DS->EleSnowCanopy[i])>0)?(tmpVar>1.0?0:1-pow(tmpVar,2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s/r_a))):0)));
//                                DS->EleET[i][1]=(beta_s1<=EPS_4)?((beta_s2<=EPS_4)?0:DS->pcCal.Et1*((1-pow(((DS->EleIS[i]+DS->EleSnowCanopy[i]<0)?0:(DS->EleIS[i]+DS->EleSnowCanopy[i]))/(DS->EleISmax[i]+DS->EleISsnowmax[i]),2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s2/r_a))))):(beta_s2<=EPS_4)?DS->pcCal.Et1*((1-pow(((DS->EleIS[i]+DS->EleSnowCanopy[i]<0)?0:(DS->EleIS[i]+DS->EleSnowCanopy[i]))/(DS->EleISmax[i]+DS->EleISsnowmax[i]),2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s1/r_a)))):DS->pcCal.Et1*((1-pow(((DS->EleIS[i]+DS->EleSnowCanopy[i]<0)?0:(DS->EleIS[i]+DS->EleSnowCanopy[i]))/(DS->EleISmax[i]+DS->EleISsnowmax[i]),2.0/3))*(pet)/(2441000000.0*(Delta+Gamma*(1+r_s/r_a))));
		...
		elemSatn=((elemSatn<EPS_4)?EPS_4:((elemSatn>1.0)?1:elemSatn));
                DS->EleET[i][2]=(DS->DummyY[i]<EPS_3)?((elemSatn>EPS_4)?elemSatn*DS->EleET[i][2]:0):DS->EleET[i][2];
...
void fluxCalc_Riv(Model_Data DS, int i,realtype t)
	{
		...
		i_plus_ele=i+DS->NumEle;
                ...
                TotalY = DS->DummyY[i_plus_totele] + DS->Riv[i].zmin;
                if(DS->Riv[i].down > 0)
                        {
                        ...
                        /************************************************************************/
                        GradCalc(DS->DummyY[inabr+DS->totele+DS->NumRiv],DS->Ele[inabr+DS->NumEle].zmin,DS->DummyY[i_plus_totele1riv],DS->Ele[i_plus_ele].zmin,DS->Riv[i].Dist[0],0,0,0,0,RIVSUB_RIVSUB);
                        ...
                        avgKH(DS->Ele[ileft].Macropore,DS->DummyY[ileft_plus_2ele],DS->Ele[ileft].zmax-DS->Ele[ileft].zmin,DS->Ele[ileft].macD,DS->Ele[ileft].macKsatH,DS->Ele[ileft].vAreaF,DS->Ele[ileft].KsatH,DS->Ele[iright].Macropore,DS->DummyY[iright_plus_2ele],DS->Ele[iright].zmax-DS->Ele[iright].zmin,DS->Ele[iright].macD,DS->Ele[iright].macKsatH,DS->Ele[iright].vAreaF,DS->Ele[iright].KsatH);
                        tmpVar=Avg_Ksat;
                        avgKH(DS->Ele[inabr_left].Macropore,DS->DummyY[inabr_left+2*DS->NumEle],DS->Ele[inabr_left].zmax-DS->Ele[inabr_left].zmin,DS->Ele[inabr_left].macD,DS->Ele[inabr_left].macKsatH,DS->Ele[inabr_left].vAreaF,DS->Ele[inabr_left].KsatH,DS->Ele[inabr_right].Macropore,DS->DummyY[inabr_right+2*DS->NumEle],DS->Ele[inabr_right].zmax-DS->Ele[inabr_right].zmin,DS->Ele[inabr_right].macD,DS->Ele[inabr_right].macKsatH,DS->Ele[inabr_right].vAreaF,DS->Ele[inabr_right].KsatH);
                        ...
                        /****************************************************************/
                #ifdef DIFFUSION
                        GradCalc(DS->DummyY[inabr + DS->totele],DS->Riv[inabr].zmin,DS->DummyY[i_plus_totele],DS->Riv[i].zmin,DS->Riv[i].Dist[0],DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->Riv_Shape[DS->Riv[inabr].shape - 1].interpOrd,DS->Riv[i].coeff,DS->Riv[inabr].coeff,RIVSURF_RIVSURF);
                #else
                        GradCalc(DS->DummyY[inabr + DS->totele],DS->Riv[inabr].zmin-DS->DummyY[inabr + DS->totele],DS->DummyY[i_plus_totele],DS->Riv[i].zmin-DS->DummyY[i_plus_totele],DS->Riv[i].Dist[0],DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->Riv_Shape[DS->Riv[inabr].shape - 1].interpOrd,DS->Riv[i].coeff,DS->Riv[inabr].coeff,RIVSURF_RIVSURF);
                #endif
...
//                      Avg_Sf = (Grad_Y>0)?Grad_Y:EPS;
                        ...
                        switch(DS->Riv[i].down)
                                {
                                case -1:
                                GradCalc(Interpolation(&DS->TSD_Riv[(DS->Riv[i].BC)-1], t),DS->Node[DS->Riv[i].ToNode-1].zmax -DS->Riv[i].depth,DS->DummyY[i_plus_totele],DS->Riv[i].zmin,DS->Riv[i].Dist[0],DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->Riv[i].coeff,DS->Riv[i].coeff,RIVSURF_RIVSURF);
...
//                              Distance = sqrt(pow(DS->Riv[i].x - DS->Node[DS->Riv[i].ToNode-1].x, 2) + pow(DS->Riv[i].y - DS->Node[DS->Riv[i].ToNode-1].y, 2));
                                Perem = CS_AreaOrPerem(DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->DummyY[i_plus_totele],DS->Riv[i].coeff,2);
                                Grad_Y = (DS->Riv[i].zmin - (DS->Node[DS->Riv[i].ToNode-1].zmax -DS->Riv[i].depth))/DS->Riv[i].Dist[0];
                                Avg_Rough = DS->Riv_Mat[DS->Riv[i].material-1].Rough;
                                Avg_Perem = Perem;
                                CrossA = CS_AreaOrPerem(DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->DummyY[i_plus_totele],DS->Riv[i].coeff,1);
                                DS->FluxRiv[i][1] = sqrt(Grad_Y)*CrossA*((Avg_Perem>0)?pow(CrossA/Avg_Perem,2.0/3.0):0)/Avg_Rough;
                                break;
                                case -4:
                                /* Critical Depth boundary conditions */
                                CrossA = CS_AreaOrPerem(DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->DummyY[i_plus_totele],DS->Riv[i].coeff,1);
                                DS->FluxRiv[i][1] = CrossA*sqrt(GRAV*UNIT_C*UNIT_C*DS->DummyY[i_plus_totele1riv]);  /* Note the dependence on physical units */
                                break;
                                default:
                ...
                if(DS->Riv[i].LeftEle > 0)
                        ...
                        /*****************************************************************************/
                        OLFeleToriv(DS->DummyY[ileft]+DS->Ele[ileft].zmax,DS->Ele[ileft].zmax,DS->Riv_Mat[DS->Riv[i].material-1].Cwr, DS->Riv[i].zmax,TotalY,DS->FluxRiv,i,2,DS->Riv[i].Length);
                        ...
                        /*********************************************************************************/
                        GradCalc(DS->DummyY[ileft_plus_2ele], DS->Ele[ileft].zmin,DS->DummyY[i_plus_totele],DS->Riv[i].zmin, DS->Riv[i].Dist[1], 0,0,0,0,RIV_ELESUB);
                        nabrAqDepth=(DS->Ele[ileft].zmax-DS->Ele[ileft].zmin);                        
			avgKH(0,0,0,0,0,0,DS->Riv[i].KsatH,DS->Ele[ileft].Macropore,DS->DummyY[ileft_plus_2ele],nabrAqDepth,DS->Ele[ileft].macD,DS->Ele[ileft].macKsatH,DS->Ele[ileft].vAreaF,DS->Ele[ileft].KsatH);
                        ...
                        /***********************************************************************************/                        
			GradCalc(DS->DummyY[ileft_plus_2ele], DS->Ele[ileft].zmin,DS->DummyY[i_plus_totele1riv],DS->Ele[i_plus_ele].zmin, DS->Riv[i].Dist[1], 0,0,DS->Riv[i].zmin,0,RIVSUB_ELESUB);                        
			avgKH(DS->Ele[ileft].Macropore,DS->DummyY[ileft_plus_2ele],nabrAqDepth,DS->Ele[ileft].macD,DS->Ele[ileft].macKsatH,DS->Ele[ileft].vAreaF,DS->Ele[ileft].KsatH,DS->Ele[iright].Macropore,DS->DummyY[iright_plus_2ele],DS->Ele[iright].zmax-DS->Ele[iright].zmin,DS->Ele[iright].macD,DS->Ele[iright].macKsatH,DS->Ele[iright].vAreaF,DS->Ele[iright].KsatH);                        
			avgKH(0,0,0,0,0,0,Avg_Ksat,DS->Ele[ileft].Macropore,DS->DummyY[ileft_plus_2ele],nabrAqDepth,DS->Ele[ileft].macD,DS->Ele[ileft].macKsatH,DS->Ele[ileft].vAreaF,DS->Ele[ileft].KsatH);
                ...
                if (DS->Riv[i].RightEle > 0)
                        ...
                        /* Lateral Surface Flux Calculation between River-Triangular element Follows */
                        /*****************************************************************************/ 
                       	OLFeleToriv(DS->DummyY[iright]+DS->Ele[iright].zmax,DS->Ele[iright].zmax,DS->Riv_Mat[DS->Riv[i].material-1].Cwr, DS->Riv[i].zmax,TotalY,DS->FluxRiv,i,3,DS->Riv[i].Length);
                        DS->FluxSurf[iright][DS->Riv[i].lrEdge[1]] = -DS->FluxRiv[i][3];                        
		#ifdef SUB_SURF_RIV
                        /*********************************************************************************/
                        /* Lateral Sub-surface Flux Calculation between River-Triangular element Follows */
                        /*********************************************************************************/
                        GradCalc(DS->DummyY[iright_plus_2ele], DS->Ele[iright].zmin,DS->DummyY[i_plus_totele],DS->Riv[i].zmin, DS->Riv[i].Dist[2], 0,0,0,0,RIV_ELESUB);
                        /* take care of macropore effect */
                        nabrAqDepth=(DS->Ele[iright].zmax-DS->Ele[iright].zmin);                        
			avgKH(0,0,0,0,0,0,DS->Riv[i].KsatH,DS->Ele[iright].Macropore,DS->DummyY[iright_plus_2ele],nabrAqDepth,DS->Ele[iright].macD,DS->Ele[iright].macKsatH,DS->Ele[iright].vAreaF,DS->Ele[iright].KsatH);                        
			DS->FluxRiv[i][5]=DS->Riv[i].Length*Avg_Ksat*Grad_Y*Avg_Y;
                        /***********************************************************************************/
                        /* Lateral Flux between rectangular element (beneath river) and triangular element */
                        /***********************************************************************************/                        
			GradCalc(DS->DummyY[iright_plus_2ele], DS->Ele[iright].zmin,DS->DummyY[i_plus_totele1riv],DS->Ele[i_plus_ele].zmin, DS->Riv[i].Dist[2],0,0,DS->Riv[i].zmin,0,RIVSUB_ELESUB);
                        /* Note this is same as that was calculated for left ele */                        
			avgKH(DS->Ele[ileft].Macropore,DS->DummyY[ileft_plus_2ele],DS->Ele[ileft].zmax-DS->Ele[ileft].zmin,DS->Ele[ileft].macD,DS->Ele[ileft].macKsatH,DS->Ele[ileft].vAreaF,DS->Ele[ileft].KsatH,DS->Ele[iright].Macropore,DS->DummyY[iright_plus_2ele],nabrAqDepth,DS->Ele[iright].macD,DS->Ele[iright].macKsatH,DS->Ele[iright].vAreaF,DS->Ele[iright].KsatH);                        
			avgKH(0,0,0,0,0,0,Avg_Ksat,DS->Ele[iright].Macropore,DS->DummyY[iright_plus_2ele],nabrAqDepth,DS->Ele[iright].macD,DS->Ele[iright].macKsatH,DS->Ele[iright].vAreaF,DS->Ele[iright].KsatH);
	...
	#ifdef SUB_SURF_RIV
		Avg_Wid=CS_AreaOrPerem(DS->Riv_Shape[DS->Riv[i].shape - 1].interpOrd,DS->DummyY[i_plus_totele],DS->Riv[i].coeff,3);                
		Dif_Y=(DS->Riv[i].zmin-(DS->DummyY[i_plus_totele1riv]+DS->Ele[i_plus_ele].zmin))>0?DS->DummyY[i_plus_totele]:DS->DummyY[i_plus_totele]+DS->Riv[i].zmin-(DS->DummyY[i_plus_totele1riv]+DS->Ele[i_plus_ele].zmin);
